...
def train():
    ...
    model_save_dir = os.path.join('logs_and_weights_add_occlusion_and_fpn_total_dataset_small_gaussian',
                                  'models',
                                  'weights',
                                  model_path_suffix)
    train_log_save_dir = os.path.join('logs_and_weights_add_occlusion_and_fpn_total_dataset_small_gaussian',
                                      'models',
                                      'logs',
                                      model_path_suffix,
                                      'train')
    test_log_save_dir = os.path.join('logs_and_weights_add_occlusion_and_fpn_total_dataset_small_gaussian',
                                     'models',
                                     'logs',
                                     model_path_suffix,
                                     'test')
...
def print_current_training_stats(global_step, cur_lr, stage_losses, total_loss, time_elapsed):
    stats = 'Step: {}/{} ----- Epoch:{:0.2f}/{} ----- Cur_lr: {:1.7f} ----- Time: {:>2.2f} sec.'.format(global_step, FLAGS.training_iters, global_step * FLAGS.batch_size / FLAGS.total_num, FLAGS.total_epoch,
                                                                                 cur_lr, time_elapsed)
    losses = ' | '.join(
